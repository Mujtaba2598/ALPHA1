...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', message: 'Auto‑Compounding Halal AI Trading Bot with User Balance Display' });
});
...
async function getFundingBalance(apiKey, secretKey) {
    try {
        const timestamp = await getServerTime();
        ...
        // If funding endpoint fails, return 0 (many API keys don't have funding wallet permission)
        return 0;
    }
}
...
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    try {
        const balance = await getSpotBalance(cleanApi, cleanSecret);
        const users = readUsers();
...
app.post('/api/connect-binance', authenticate, async (req, res) => {
    ...
    try {
        const spotBalance = await getSpotBalance(apiKey, secretKey);
        const fundingBalance = await getFundingBalance(apiKey, secretKey);
        res.json({ 
            success: true, 
...
class AutoCompoundingEngine {
    async executeTrade(sessionId, userEmail, apiKey, secretKey, config) {
        ...
        const actualBalance = await getSpotBalance(apiKey, secretKey);
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    ...
    try {
        const balance = await getSpotBalance(apiKey, secretKey);
...
app.post('/api/get-balance', authenticate, async (req, res) => {
    ...
    try {
        ...
        const spotBalance = await getSpotBalance(apiKey, secretKey);
        const fundingBalance = await getFundingBalance(apiKey, secretKey);
        res.json({ success: true, spotBalance, fundingBalance, total: spotBalance + fundingBalance });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Owner can view all users' Spot + Funding balances`);
